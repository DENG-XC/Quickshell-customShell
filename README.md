...
> [!NOTE]
> This project has only been tested on a 2K monitor and 1K monitor. Compatibility with other resolutions or multi-monitor setups is not guaranteed.
...
```kdl
// ~/.config/niri/config.kdl
spawn-at-startup "awww-daemon" // run awww-daemon on startup
...
- Only tested on 2K monitor and 1K monitor
...
> [!NOTE]
> 本项目仅在单屏 2K 分辨率 和 1K 分辨率下测试过，不保证在其他分辨率或多屏环境下的兼容性。
...
```kdl
// ~/.config/niri/config.kdl
spawn-at-startup "awww-daemon" // 启动 awww-daemon
